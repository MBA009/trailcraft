...
export default function Cart({ onClose }){
  ...
  const applyPromo = async () => {
    ...
    try {
      const res = await api.get(`${url}/api/promos/validate/${promoInput.trim()}`)
        ...
        {order && (
          <div className="flex-1 flex flex-col items-center justify-center gap-4 px-6">
            ...
            <div className="text-center">
              ...
              {order.promoCode && <p className="text-xs text-green-600 mt-1">Promo <span className="font-bold">{order.promoCode}</span> applied</p>}
            </div>
            <button onClick={() => { setOrder(null); onClose?.() }}
                ...
                <div key={i.cartKey} className="flex gap-4 p-3 rounded-xl border border-gray-100 hover:border-gray-200 transition-all">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-2">
                      <button onClick={() => updateQty(i.cartKey, i.qty - 1)}
                        className="w-6 h-6 rounded-full border border-gray-200 flex items-center justify-center text-gray-500 hover:border-gray-900 hover:text-gray-900 transition-all text-sm">−</button>
            ...
            {items.length > 0 && (
              <div className="border-t border-gray-100 px-4 sm:px-6 py-4 space-y-3 bg-gray-50">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <input name="name" value={shipping.name} onChange={handleChange} placeholder="Full name" className="col-span-2 border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-orange-400 bg-white" />
                  <input name="email" value={shipping.email} onChange={handleChange} placeholder="Email" className="col-span-2 border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-orange-400 bg-white" />
                  <input name="address" value={shipping.address} onChange={handleChange} placeholder="Address" className="col-span-2 border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-orange-400 bg-white" />
                  <input name="city" value={shipping.city} onChange={handleChange} placeholder="City" className="border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-orange-400 bg-white" />
                  <input name="postalCode" value={shipping.postalCode} onChange={handleChange} placeholder="Postal code" className="border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-orange-400 bg-white" />
                  <input name="country" value={shipping.country} onChange={handleChange} placeholder="Country" className="col-span-2 border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-orange-400 bg-white" />
                </div>
                ...
                {!promoCode ? (
                  <div>
                    <div className="flex gap-2">
                      <input
                        ...
                        className="flex-1 border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-orange-400 bg-white uppercase"
                        onKeyDown={e => e.key === 'Enter' && applyPromo()}
                      />
                      <button onClick={applyPromo} disabled={promoLoading || !promoInput.trim()}
                        className="text-sm font-semibold text-orange-500 border border-orange-200 hover:bg-orange-50 px-3 py-2 rounded-lg transition-all disabled:opacity-40">
                        {promoLoading ? '...' : 'Apply'}
                      </button>
                    </div>
                  ...
                  <div className="flex items-center justify-between bg-green-50 border border-green-100 rounded-lg px-3 py-2">
                    <p className="text-xs text-green-700 font-semibold">🎉 {promoCode} — {promoDiscount}% off applied</p>
                    <button onClick={removePromo} className="text-xs text-gray-400 hover:text-gray-600">Remove</button>
                  </div>
                )}
                ...
                <div className="flex items-center justify-between pt-1">
                  <div>
                    ...
                    <p className="text-xs text-gray-400">Total{promoDiscount > 0 ? ` (−$${discountAmount.toFixed(2)})` : ''}</p>
