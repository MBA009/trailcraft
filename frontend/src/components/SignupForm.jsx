...
export default function SignupForm({ onClose, defaultRole = 'customer', onSwitchToLogin }){
  ...
  const submit = async (e) => {
    ...
    if (isVendor && !storeName.trim()) { setErr('Store name is required'); return }
    setLoading(true); setErr(null)
    try {
      await register({ name, email, password, role: defaultRole, ...(isVendor ? { storeName } : {}) })
      onClose?.()
  ...
  return (
    <div className="fixed inset-0 bg-black/50 flex items-end sm:items-center justify-center z-50 sm:p-4" onClick={onClose}>
      <div className="bg-white rounded-t-2xl sm:rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden max-h-[95vh] flex flex-col" onClick={e => e.stopPropagation()}>
        <div className={`h-1.5 bg-gradient-to-r flex-shrink-0 ${isVendor ? 'from-gray-700 to-gray-900' : 'from-orange-400 to-orange-600'}`} />
        <div className="p-6 sm:p-8 overflow-y-auto">
          ...
          <div className="text-center mb-7">
            ...
            {isVendor ? (
              <>
                <div className="mt-3 inline-flex items-center gap-1.5 bg-gray-900 text-white text-xs font-semibold px-3 py-1 rounded-full">
                  <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-1.5 6h11"/></svg>
                  Vendor Account
                </div>
                <p className="text-gray-500 text-sm mt-2">Start selling your shoes today</p>
              </>
            ) : (
          ...
          <form onSubmit={submit} className="space-y-4">
            ...
            {isVendor && (
              <div>
                <label className="block text-xs font-semibold text-gray-500 mb-1.5 uppercase tracking-wide">Store name</label>
                <input value={storeName} onChange={e => setStoreName(e.target.value)} placeholder="My Shoe Store"
                  className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-orange-400 focus:ring-2 focus:ring-orange-100 transition-all" />
              </div>
            )}
            ...
            <button type="submit" disabled={loading}
              className={`w-full disabled:opacity-60 text-white font-bold py-3 rounded-xl transition-all text-sm shadow-sm mt-2 ${isVendor ? 'bg-gray-900 hover:bg-gray-700' : 'bg-orange-500 hover:bg-orange-600'}`}>
          ...
          <p className="text-center text-sm text-gray-500 mt-6">
            ...
            <button onClick={() => { onClose?.(); onSwitchToLogin?.() }} className="text-orange-500 font-semibold hover:text-orange-600">
              Sign in
            </button>
          </p>
        </div>
